...
extern int selected_landmark_num;
extern std::vector<int> five_landmarks;
extern std::vector<cv::Point> five_landmarks_coord;
...
namespace LandmarkDetector
{
	...
	void set_draw_params(bool is_draw_bbox_, bool is_draw_pyramid_,
		bool is_draw_face_landmarks_, bool is_draw_pyramid_landmarks_)
	{
		is_draw_bbox = is_draw_bbox_;
		is_draw_pyramid = is_draw_pyramid_;
		is_draw_face_landmarks = is_draw_face_landmarks_;
		//is_draw_5_landmarks = is_draw_5_landmarks_;
		is_draw_pyramid_landmarks = is_draw_pyramid_landmarks_;
	};
	...
	void DrawBox(cv::Mat image, cv::Vec6d pose, cv::Scalar color, int thickness, float fx, float fy,
		float cx, float cy, std::vector<cv::Point>& callibration_pnt,
		std::vector<cv::Point>& callibration_pnt_planar)
	{
		double boxVerts[] = {
			...
			-1, -1, -1,   //6
			-1, -1, 1,    //7
			0, 0,  1,     //8 - Added adition middle points on front box face 
			1,	1, 0,     //9 - flat bbox (anti-spoofing) 
			1,-1, 0,      //10
			0,0,-1 };     //11

		std::vector<int> bbox_3D_landmarks = { 0,1,5,9,10 };
		std::vector<int> bbox_2D_landmarks = { 11,6,0,1,5};
		...
		cv::Mat_<double> box = cv::Mat(12, 3, CV_64F, boxVerts).clone() * 100; //We added addition vertex at the center of front face

		cv::Matx33d rot = LandmarkDetector::Euler2RotationMatrix(cv::Vec3d(pose[3], pose[4], pose[5]));
		//'rot' is the rotation matrix computed from Euler angles
		cv::Mat_<double> rotBox;

		// Rotate the box
		rotBox = cv::Mat(rot) * box.t(); // rotBox*rot is corresponding rotation of 8 unit bbox vertixes
		rotBox = rotBox.t();			 // put vertex coordiantes in rows

										 // Move the bounding box to head position
		rotBox.col(0) = rotBox.col(0) + pose[0]; //each column is x,y,z coord of 8 vertixes, we move tham accoridng to 
		rotBox.col(1) = rotBox.col(1) + pose[1]; // obtianed translation vector 'pos'
		rotBox.col(2) = rotBox.col(2) + pose[2];

		// draw the lines
		cv::Mat_<double> rotBoxProj;
		Project(rotBoxProj, rotBox, fx, fy, cx, cy); //fxy is a  focal distance, cxy are camera centers
													 //rotBoxProj is 8 2D vertices of projected bbox.  
		...
		for (size_t i = 0; i < edges.size(); ++i)
		{
			...
			rotBoxProj.row(edges[i].first).copyTo(begin); //begin,end is 2d coord of edge note for drawing
			...
			if (is_draw_bbox & ( p1.inside(image_rect) || p2.inside(image_rect) ))
			{
				cv::line(image, p1, p2, color, thickness, CV_AA, draw_shiftbits);
			}
		}

		for (auto i : bbox_3D_landmarks) {
			cv::Point p(cvRound((rotBoxProj.row(i)).at<double>(0) * (double)draw_multiplier), cvRound((rotBoxProj.row(i)).at<double>(1) * (double)draw_multiplier));
			callibration_pnt.push_back(p);
			//cv::circle(image, p, 1 * draw_multiplier, cv::Scalar(50, 50, 50), thickness, CV_AA, draw_shiftbits);
		}

		for (auto i : bbox_2D_landmarks) {
			//std::cout << "i=" << i;
			cv::Point p(cvRound((rotBoxProj.row(i)).at<double>(0) * (double)draw_multiplier), cvRound((rotBoxProj.row(i)).at<double>(1) * (double)draw_multiplier));
			callibration_pnt_planar.push_back(p);
			//cv::circle(image, p, 0.7 * draw_multiplier, cv::Scalar(100, 100, 0), thickness, CV_AA, draw_shiftbits);
		}
	}
	...
	//anti-spoofing
	void DrawPyramid(cv::Mat image, cv::Vec6d pose, cv::Scalar color, int thickness, float fx, float fy,
			float cx, float cy, std::vector<cv::Point>& callibration_pnt,
			const Pyramid& P, bool enable_drawing )

	{

		double boxVerts[] = {
			P.x1, P.y1,  P.z,       //0  front face (antispoofing)
			P.x1, P.y2,  P.z,       //1
			P.x2, P.y2,  P.z,       //2
			P.x2, P.y1,  P.z,       //3
			(P.x1+ P.x2)/2, (P.y1 + P.y2) / 2, P.z+P.h};     //4 pyramid point


		std::vector<int> bbox_3D_landmarks = { 4,0,1,2,3};
		...
		edges.push_back(pair<int, int>(2, 3));
		edges.push_back(pair<int, int>(0, 3));
		edges.push_back(pair<int, int>(0, 4));
		...
		// The size of the head is roughly 200mm x 200mm x 200mm
		cv::Mat_<double> box = cv::Mat(5, 3, CV_64F, boxVerts).clone() * 100; //We added addition vertex at the center of front face
		//std::cout << "\nbbox=\n" << box;
		...
		// draw the lines
		cv::Mat_<double> rotBoxProj;
		Project(rotBoxProj, rotBox, fx, fy, cx, cy); //fxy arecmaera focal distance, cxy are camera centers
		//rotBoxProj is 8 2D vertices of projected bbox.  
		cv::Rect image_rect(0, 0, image.cols * draw_multiplier, image.rows * draw_multiplier);
		...
		if (is_draw_pyramid)
			for (size_t i = 0; i < edges.size(); ++i)
			{
				...
				// Only draw the line if one of the points is inside the image
				if (is_draw_pyramid & enable_drawing & (p1.inside(image_rect) || p2.inside(image_rect)) )
				{
					cv::line(image, p1, p2, color, thickness, CV_AA, draw_shiftbits);
				}
			}
		...
		for (auto i : bbox_3D_landmarks) {
			cv::Point p(cvRound((rotBoxProj.row(i)).at<double>(0) * (double)draw_multiplier), cvRound((rotBoxProj.row(i)).at<double>(1) * (double)draw_multiplier));
			callibration_pnt.push_back(p);
			if (is_draw_pyramid_landmarks & enable_drawing)
				cv::circle(image, p, 1 * draw_multiplier, cv::Scalar(50, 50, 50), thickness, CV_AA, draw_shiftbits);
		}
	}
	
	//anti-spoofing
	void DrawFivePoints(cv::Mat image, cv::Vec6d pose, cv::Scalar color, int thickness, float fx, float fy,
		float cx, float cy, std::vector<cv::Point>& callibration_pnt,
		const MatrixD& P, bool enable_drawing)

	{

		double boxVerts[] = {
			P[0][0], P[0][1], P[0][2],       //0  front face (anti-spoofing)
			P[1][0], P[1][1], P[1][2],       //1 
			P[2][0], P[2][1], P[2][2],       //2 
			P[3][0], P[3][1], P[3][2],       //3 
			P[4][0], P[4][1], P[4][2]  };    //4 pyramid point
																   ...
																   //	 1, 1,   -1,       //2
																   //	 1, -1,  -1,       //3
																   //	 0, 0, width - 1 };  //4 pyramid point

																   //std::vector<int> bbox_3D_landmarks = { 0,3,1,2,4 };
		std::vector<int> bbox_3D_landmarks = { 4,0,1,2,3 };
	...
	double get_cp1(std::vector<cv::Point>& XY) {
		//std::cout << "\n" << ",getM(XY, 1, 2, 4)" << getM(XY, 1, 2, 4) << ",getM(XY, 1, 3, 5)" << getM(XY, 1, 3, 5);
		//std::cout << ",getM(XY, 1, 2, 5)" << getM(XY, 1, 2, 5) << "getM(XY, 1, 3, 4)=" << getM(XY, 1, 3, 4);

		//return (getM(XY, 0, 1, 3) * getM(XY, 0, 2, 4)) / (getM(XY, 0, 1, 4)*getM(XY, 0, 2, 3)); //index starts from 0 not 1 !!!
		return (getM(XY, 1, 2, 4) * getM(XY, 1, 3, 5)) / (getM(XY, 1, 2, 5)*getM(XY, 1, 3, 4));//1st point is a middle point 
	}

	double getM(std::vector<cv::Point>& P, int i, int j, int k) {
		cv::Mat_<double> M(3, 3);
		//M << P[i].x, P[j].x, P[k].x, P[i].y, P[j].y, P[k].y, 1.0, 1.0, 1.0;
		M << P[i-1].x, P[j-1].x, P[k-1].x, P[i-1].y, P[j-1].y, P[k-1].y, 1.0, 1.0, 1.0;
		//=  (cv::Mat_<double>(3,3) << P[i].x ,P[j].x,P[k].x,  P[i].y, P[j].y, P[k].y,  1,1,1;
		...
		if (n >= 66)
		{
			for (int i = 0; i < n; ++i)
			{
				if (visibilities.at<int>(i))
				{
					...
					cv::Scalar circ1_color(0, 0, 255), circ2_color(255, 0, 0);

					auto landmark_it = std::find(five_landmarks.begin(), five_landmarks.end(), i);
					if (landmark_it != five_landmarks.end()) {
						int ind = landmark_it - five_landmarks.begin();
						five_landmarks_coord[ind] = featurePoint;
						circ1_color = cv::Scalar(0, 255, 0);
						circ2_color = cv::Scalar(255, 255, 0);
					}
					if (i == selected_landmark_num) {//choosing 5 landmark points
						circ1_color = cv::Scalar(255, 255, 255);
						circ2_color = cv::Scalar(255, 255, 0);
					}

					if (is_draw_face_landmarks) {
						cv::circle(img, featurePoint, 1 * draw_multiplier, circ1_color, thickness, CV_AA, draw_shiftbits);
						cv::circle(img, featurePoint, 1 * draw_multiplier, circ2_color, thickness_2, CV_AA, draw_shiftbits);
					}

					string landmark_num = std::to_string(i);
				}
			}
		}
